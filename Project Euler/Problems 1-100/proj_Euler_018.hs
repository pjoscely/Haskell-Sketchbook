...
Brute force solution using binary 0 -> Left 1 -> Right to test all paths
max = -1
for i in range(2**14):
    path = dec_to_bin(i)
    row_index = 1
...
answer :: IO Int
answer = do
  x <- readFile "p_018.txt"
  -- result -> [[75],[95,64],[17,47,82],[18,35,87,10],...,[4,62,98,27,23,9,70,98,73,93,38,53,60,4,23]]
  let result = map (\l -> map (\x -> read x ::Int ) $ words l ) $ lines x
  let final = solve result
  return final

{-Dynamic solution working up from the last row
3              3               3          23
7 4            7 4             20 18  => 
2 4 6    =>    10 13 14    => 
8 5 9 3        
-}
solve :: (Foldable t, Num a, Ord a) => t [a] -> a
